...
现在这套 skill 的目标进一步收敛为：默认产出的 PRD 不只是“结构完整”，而是更接近能直接支撑 `V1` 落地评审的多文档骨架。判断标准不只是有没有章节，而是：
...
这套 skill 的总目标不是生成“第一轮结构化成稿”，而是把一个产品想法推进为一套可直接用于评审和落地的产品交付物：

1. 先经过充分头脑风暴，展开用户、场景、问题、备选方案、关键假设与边界。
2. 再经过一轮符合乔布斯产品思维的收敛判断，明确真正要做什么、不做什么、核心体验是什么、为什么现在做。
3. 最终产出一套内容详细、结论明确、跨章节打通的多文档 PRD。
4. 这套 PRD 应当能直接用于产品评审、研发拆解、版本规划、里程碑推进和开发落地。

## 当前成稿标准

核心 PRD 页面默认要能分别支撑三类会议，而不是只服务阅读：

| 会议 | 主要章节 | 文档要解决的问题 |
| --- | --- | --- |
| 产品评审会 | `02` `03` `05` `12` | 为什么做、做什么、不做什么、怎么切版本 |
| 研发拆解会 | `06` `07` `08` `10` `14` | 用户流程、功能需求、规则边界、技术边界、数据方案 |
| 里程碑会 | `11` `15` `16` `17` | 风险依赖、上线门槛、节奏计划、待拍板事项 |

如果某页只能帮助“继续讨论”，还不能支持“拍板 / 拆解 / 排期”，就还没有达到本 skill 的目标质量。

## 完成定义 / DoD

只有当下面这些条件成立时，才应认为这个 skill 完成了本次交付：

| 条件 | 完成标准 |
| --- | --- |
| 已完成头脑风暴 | 有明确的用户、场景、问题、方案备选、推荐方案、关键假设 |
| 已完成 Jobs 收敛 | 有一句话定义、Focus / Say No、核心体验原则、该砍掉什么 |
| PRD 可评审 | 能直接支撑产品评审会，不只是信息罗列 |
| PRD 可拆解 | 研发可据此拆解功能、规则、接口、数据、验收项 |
| PRD 可落地 | 已覆盖版本切分、上线门槛、关键路径、待拍板事项 |
| 跨章节打通 | 章节之间有输入输出关系，不是 18 篇孤立文档 |
| 图表有效 | 图表和表格真正帮助评审，不是装饰 |

如果只是“文档很多、章节很全”，但还不能支持评审、拆解和落地，就不算完成。
...
这个 skill 默认按“尽量一次对话跑完整条链路”工作。

也就是说，如果你没有明确说“只做 PRD”或“先停在分析阶段”，代理不应该在下面这些中间节点停住：
...
```text
帮我一次性完成完整交付：analysis + 多文档 PRD + 飞书知识库发布准备。如果没有阻塞，不要中途停下来问我是否继续。
```

如果你只想要某一段，也建议直接说：

```text
只做本地多文档 PRD，不要继续原型和飞书发布。
```
...
6. 如果发布到飞书知识库，则一级节点文档正文开头加上目录，方便知识库内阅读和跳转
