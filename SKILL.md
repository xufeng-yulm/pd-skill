...
1. 全网调研完成
   - 已产出 `brief/01-web-research.md`
   - 四类维度（市场格局 / 用户痛点 / 行业趋势 / 技术方案）均已有数据，每类至少 2 条带 URL + 抓取日期的结论
   - 已收敛出 3-5 条「必须带入头脑风暴」的核心输入（写在文档第 3 节）
...
2. 需求分析完成
   ...
   - 「Why Now?」「方案备选」「关键假设」段已显式回引 `brief/01-web-research.md` 第 3 节的核心输入
...
- `WebSearch` / `WebFetch`（全网深度调研）
  来源：模型内置工具
  用途：在头脑风暴前做全网信号扫描，覆盖市场格局、用户痛点、行业趋势、技术方案四类维度；产出 `brief/01-web-research.md`。
...
0. `WebSearch` / `WebFetch`（全网深度调研）
   - 对应阶段：需求调研（头脑风暴前）
   - 最低产物：`brief/01-web-research.md`
   - 约束：未对市场格局 / 用户痛点 / 行业趋势 / 技术方案四类维度做实际检索并沉淀带 URL + 抓取日期的结论，不得宣称全网调研完成，也不得进入头脑风暴
...
这是头脑风暴**之前**的强制前置步骤，目的是把外部证据（竞品、用户痛点、行业趋势、技术方案）沉淀到本地，避免 PM 闭门造车、方案备选缺乏事实支撑。

使用工具：

- `WebSearch` 做关键词检索（中英文双语，覆盖中英文资料、监管文件、社区贴）。
- `WebFetch` 抓取一级来源（官网、招股书、监管文件、官方公告）和关键二级来源（媒体报道、社区贴）。
- 不要机械使用 LLM 训练记忆替代实际检索——所有外部结论必须能追到 URL。

最低产物：

- `brief/01-web-research.md`

必须覆盖四类维度（每类至少 2 条结论、1 条核心数据、1 个出处）：

1. **市场格局与竞品扫描**——赛道 Top 玩家、定位/定价/份额、差异化机会点。
2. **用户痛点与需求信号**——公开渠道（社区/评论/调研/客服脚本）反复抱怨的痛点；现有替代方案的失灵场景。
3. **行业趋势与最近事件**——近 12 个月同类产品的融资/收购/新发布、监管政策变化、技术标准升级。
4. **技术可行性与成熟方案**——主流技术栈和开源方案、被验证过的架构、关键技术风险与合规门槛。

调研问题清单、表格模板、引用规范详见 `brief/01-web-research.md` 的初始化内容（由 `scripts/init_pm_case.py` 生成）。

执行约束：

1. **每条结论都必须给「URL + 抓取日期」**。没有出处的数据点不算结论。
2. **关键结论至少 1 个一级来源或 2 个二级来源**。禁止只引用百科类条目。
3. 调研过程是发散—收敛两阶段：先广撒网（每个维度 5-10 个查询）抓信号，再收敛成 3-5 条**必须带入头脑风暴**的输入。
4. 把「必须带入头脑风暴的 3-5 条输入」单独写到 `brief/01-web-research.md` 的第 3 节，头脑风暴阶段会强制回引。
5. 如果某类维度确实搜不到证据，写明「已尝试关键词 X / Y / Z，公开渠道未找到一级来源」，不要伪造数据。
6. 调研本身可以**并行**：四个维度可以同时跑 `WebSearch`，但同一维度的多个查询需要保持去重。

完成定义：

- `brief/01-web-research.md` 已写到「## 3. 给头脑风暴的关键输入」并收敛出 3-5 条核心输入。
- 四类维度都有数据；任何一类只有空表，调研步骤不算完成。
- 至少 8 条带 URL + 抓取日期的结论。
...
- `brief/01-web-research.md`（强制前置，必须先完成）
...
头脑风暴阶段必须**显式引用** `brief/01-web-research.md` 第 3 节的 3-5 条核心输入；这些输入至少要进入：

- 「Why Now?」段：引用「行业趋势与最近事件」「市场窗口」类输入。
- 「方案备选」「推荐方案」段：引用「竞品空白」「用户未被满足的需求」类输入。
- 「关键假设」「待确认问题」段：引用「必须警惕的外部风险」「技术成熟度」类输入。
